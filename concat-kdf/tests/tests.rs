...
fn test_input_output<D>(tests: &[Test])
where
    D: Digest + FixedOutputReset,
{
    ...
    } in tests.iter()
    {
        for key_length in 1..expected_key.len() {
            ...
            assert!(concat_kdf::derive_key_into::<D>(secret, other_info, &mut key).is_ok());
...
fn test_input_output_sha224() {
    ...
    test_input_output::<Sha224>(&tests);
}
...
fn test_input_output_sha256() {
    ...
    test_input_output::<Sha256>(&tests);
}
...
fn test_input_output_sha512() {
    ...
    test_input_output::<Sha512>(&tests);
}
